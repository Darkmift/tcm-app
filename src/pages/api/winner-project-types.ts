...
import {WinnerProjectType, InsertWinnerProjectType} from '@/types'
import {COLLECTIONS} from '@/const'
// import

// Handler to get all winner project types
const getAllWinnerProjectTypes = async (
  req: NextApiRequest,
  res: NextApiResponse<
    {winnerProjectTypes: WinnerProjectType[]} | {error: string}
  >
) => {
  try {
    const winnerProjectTypes = await pocketDbService.getCollection(
      COLLECTIONS.WINNER_PROJECT_TYPES
    )
    res.status(200).json({winnerProjectTypes})
  } catch (error) {
